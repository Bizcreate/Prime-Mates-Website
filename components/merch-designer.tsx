...
import { Download, Layers, Trash2, Upload, Wand2 } from "lucide-react";
...
type Product = {
  ...
  area: { x: number; y: number; w: number; h: number }; // relative 0..1
};
...
// tiny chroma-key by distance in RGB
...
export default function MerchDesigner({ userNFTs }: { userNFTs: NFTData[] }) {
  ...
  function addLayerFromSrc(src: string, label?: string) {
    ...
    const l: Layer = {
      ...
      scale: 0.6,
      rotation: 0,
      removeBg: { enabled: false, color: "#ffffff", tol: 60 },
    };
    setLayers((p) => [...p, l]);
    setSelectedId(id);
  }
  ...
  async function exportPNG() {
    ...
    try {
      ...
      for (const l of layers) {
        const src =
          l.removeBg?.enabled ? await colorKey(l.src, l.removeBg.color, l.removeBg.tol) : l.src;
        const img = await loadImage(src);
        const px = l.x * scaleX;
        const py = l.y * scaleY;
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    setLayers((prev) =>
      prev.map((l) => (l.id === down.id ? { ...l, x: l.x + dx, y: l.y + dy } : l)),
    );
      ...
      <div>
          ...
          {layers.map((l) => (
            <img
              ...
              className={`absolute cursor-grab ${
                selectedId === l.id ? "ring-2 ring-yellow-400" : ""
              }`}
              style={{
                left: l.x,
                top: l.y,
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          <label className="inline-flex items-center gap-2">
            ...
            <span className="px-3 py-2 rounded-lg bg-gray-900 border border-gray-800 cursor-pointer">
              Upload image
            </span>
          </label>
          ...
          <CardContent className="p-4">
            ...
            {userNFTs.length === 0 ? (
              <div className="text-sm text-gray-400">
                Connect wallet on the “My Collection” tab to load NFTs.
              </div>
            ) : (
              <div className="grid grid-cols-3 gap-2 max-h-56 overflow-y-auto pr-1">
                {userNFTs.map((n) => {
                  const thumb = resolveMediaUrl(n.image) ?? "/placeholder.png";
                  return (
                    <button
                      key={`${n.tokenAddress ?? "na"}-${n.tokenId}`}
                      onClick={() => addLayerFromSrc(thumb, n.name)}
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div>
                ...
                <Slider
                  ...
                  step={0.02}
                  onValueChange={([v]) =>
                    setLayers((p) => p.map((l) => (l.id === selected.id ? { ...l, scale: v } : l)))
                  }
                />
              </div>
              <div>
                ...
                <Slider
                  ...
                  step={1}
                  onValueChange={([v]) =>
                    setLayers((p) => p.map((l) => (l.id === selected.id ? { ...l, rotation: v } : l)))
                  }
                />
              </div>
                        ...
                        p.map((l) =>
                          l.id === selected.id
                            ? {
                                ...l,
                                removeBg: {
                                  ...(l.removeBg || { color: "#ffffff", tol: 60 }),
                        ...
                        p.map((l) =>
                          l.id === selected.id
                            ? {
                                ...l,
                                removeBg: {
                                  ...(l.removeBg || { enabled: true, tol: 60 }),
                        ...
                        p.map((l) =>
                          l.id === selected.id
                            ? {
                                ...l,
                                removeBg: {
                                  ...(l.removeBg || { enabled: true, color: "#ffffff" }),
                                  tol: Number(e.target.value) || 60,
                                },
                              }
                            : l,
                        ),
