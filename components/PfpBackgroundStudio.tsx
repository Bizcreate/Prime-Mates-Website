...
function buildMask(data: ImageData, bgColors: RGB[], threshold = 30, feather = 2): Uint8ClampedArray {
  const { width: W, height: H, data: A } = data
      ...
      <div className="space-y-6">
        <div className="p-4 rounded-xl border border-gray-800 bg-gray-900">
          ...
          <div className="mt-2">
            ...
            <input
              ...
              max={5}
              value={feather}
              onChange={(e) => setFeather(Number.parseInt(e.target.value))}
              className="w-full"
            />
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-gray-800 bg-gray-900 space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <button
              className={`px-2 py-1 rounded border ${bgGradient ? "border-yellow-500 text-yellow-400" : "border-gray-700 text-gray-300"}`}
              onClick={() => setBgGradient(bgGradient ? null : gradients[0].css)}
            >
        ...
        <div className="p-4 rounded-xl border border-gray-800 bg-gray-900 space-y-2">
          ...
          <input
            ...
            step={1}
            value={offsetX}
            onChange={(e) => setOffsetX(Number.parseInt(e.target.value))}
            className="w-full"
          />
          ...
          <input
            ...
            step={1}
            value={offsetY}
            onChange={(e) => setOffsetY(Number.parseInt(e.target.value))}
            className="w-full"
          />
        </div>
        ...
        <div className="p-4 rounded-xl border border-gray-800 bg-gray-900 space-y-2">
          ...
          <input
            className="w-full bg-gray-800 border border-gray-700 rounded-lg p-2"
            placeholder="/gestures/ShakaHand.png"
            value={gesture || ""}
            onChange={(e) => setGesture(e.target.value || null)}
          />
        </div>

        <button
          onClick={onDownload}
          className="w-full rounded-xl bg-yellow-500 text-black font-semibold py-2 hover:opacity-90"
        >
          Download PNG
        </button>
      </div>
